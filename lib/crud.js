...
module.exports = {
  ...
  create: async function (connection, tableName, data) {
    const createdRecords = await pcsapi.connections[connection](tableName)
      .insert(data)
      .returning('*')
...
async function findRecords(
  ...
  tableName,
  filter,
  { findOne = false, noSelects = false, modifyFn } = {}
) {
  ...
  iterateFilterCriteria(query, filter, tableName, !noSelects)
  if (filter.limit) {
    iterateFilterCriteria(limitQuery, filter, tableName)
  ...
  if (_.isFunction(modifyFn)) {
    query.modify(modifyFn, tableName)
  }
  if (findOne) {
    iterateFilterCriteria(limitQuery, filter, tableName)
    const firstId = await limitQuery
      .first(`${tableName}.id`)
      .then(record => (record ? record.id : 0))
...
function iterateFilterCriteria(query, filter, tableName, applySelects = false) {
  _.map(filter, function (value, key) {
    ...
    switch (true) {
      ...
      case key === 'select':
        if (applySelects) {
          const selectAttributes = _.uniq(
            _.map(isString ? value.split(',') : value, function (attribute) {
      ...
      case isObject && !isArray:
        _.each(value, function (nestedValue, nestedKey) {
          if (nestedKey === '!') {
            query.whereNotIn(
              `${tableName}.${key}`,
              _.isArray(nestedValue) ? nestedValue : [nestedValue]
            )
          } else {
