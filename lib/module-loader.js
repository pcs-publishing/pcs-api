...
module.exports = {
  load: function () {
    ...
    loadServices(function (err, modules) {
      ...
      pcsapi.log.silly(
        `module-loader.js - load - Loaded services and exposed globally`
      )
    })
    ...
    function loadServices(cb) {
      pcsapi.log.verbose(
        `module-loader.js - load - Loading services from ${process.cwd()}/api/services`
      )
    ...
    function loadControllers(cb) {
      pcsapi.log.verbose(
        `module-loader.js - load - Loading controllers from ${path.resolve(
          './api/controllers/'
        )}`
      )
  ...
  loadServices: function (servicesPath) {
    pcsapi.log.verbose(
      `module-loader.js - loadServices - Loading services from ${servicesPath}`
    )
  ...
  loadMixins: function (mixinsPath) {
    pcsapi.log.verbose(
      `module-loader.js - loadMixins - Loading mixins from ${mixinsPath}`
    )
...
function loadModulesFromPath(servicesPath, cb) {
  pcsapi.log.verbose(
    `module-loader.js - loadModulesFromPath - Loading services from ${servicesPath}`
  )
  includeAll.optional(
    {
...
function bindToHelper(cb) {
  return function (err, modules) {
    if (err) {
      pcsapi.log.error('Failed to load modules. Details:', err)
      return cb(err)
    }
