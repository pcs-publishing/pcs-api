...
module.exports = {
  ...
  uniqueBy: function (array, handler, mergeNestedArrays) {
    ...
    return _.filter(
      array,
      function (item, index) {
        const valueOfUniqueProperty = _.isString(handler)
          ? objectUtil.getValueAt(item, handler)
          : handler(item, index, array)

        if (
          !valueUtil.hasValue(valueOfUniqueProperty) ||
          _.includes(uniqueValuesFound, valueOfUniqueProperty)
        ) {
          if (mergeNestedArrays) {
            ...
            if (!_.isEmpty(arrayKeys)) {
              const uniqueObject = _.find(array, function (arrayItem) {
                const uniqueObjectValue = _.isString(handler)
                  ? objectUtil.getValueAt(arrayItem, handler)
                  : handler(arrayItem, index, array)
              ...
              _.each(
                ...
                function (key) {
                  ...
                  uniqueObject[key] = _.isObject(uniqueObject[key][0])
                    ? array.uniqueBy(uniqueObject[key], handler)
                    : _.unique(uniqueObject[key])
                },
                this
              )
            }
