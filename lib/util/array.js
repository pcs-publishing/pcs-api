...
module.exports = {
  compactMap: pcsutils.array.compactMap,
  containsAll: pcsutils.array.containsAll,
  containsAny: pcsutils.array.containsAny,
  toArray: pcsutils.array.toArray,
    ...
    return _.filter(array, function (item, index) {
      ...
      if (!valueUtil.hasValue(valueOfUniqueProperty) || _.includes(uniqueValuesFound, valueOfUniqueProperty)) {
        if (mergeNestedArrays) {
          const arrayKeys = pcsutils.array.compactMap(item, function (value, key) {
            if (_.isArray(value)) {
              return key
            }
          })
  ...
  compact: function (array) {
    ...
    return pcsutils.array.compactMap(arrayToCompact, function (arrayElement) {
