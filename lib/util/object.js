...
module.exports = {
  ...
  /**
   * Are the two objects the same when only comparing the passed properties
   * @param {Object} obj1 The first object to use in the comparison
   * @param {Object} obj2 The second object to use in the comparison
   * @param {String[]} properties The properties to use as comparators on the objects
   *
   * @return {Boolean} Are the two objects the same when looking at only the passed properties
   */
  areObjectsTheSame: function (obj1, obj2, properties) {
    const pickedObj1 = _.pick(obj1, properties)
    const pickedObj2 = _.pick(obj2, properties)
